...
class Staff(Base):
    ...
    email = Column(String(150), unique=True, index=True, nullable=False)
    password_hash = Column(String(255), nullable=False)
    domain = Column(Enum(CategoryEnum), nullable=False)  # Structural, Functional, Performance
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc), nullable=False)
...
class Complaint(Base):
    ...
    severity = Column(Float, default=1.0)  # Severity score 1.0 - 10.0
    extent = Column(Float, default=1.0)    # Visible defect extent (0.0 - 100.0%)
